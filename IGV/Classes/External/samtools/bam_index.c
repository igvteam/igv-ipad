...
static bam_index_t *bam_index_load_core(FILE *fp)
{
	...
	for (i = 0; i < idx->n; ++i) {
		...
		for (j = 0; j < (int)size; ++j) {
            ...
            // TODO - dat - LEAK LEAK LEAK
            // TODO - dat - LEAK LEAK LEAK
//            p->list = (pair64_t*)malloc(p->m * 16);
            p->list = (pair64_t*)calloc(p->m, sizeof(pair64_t));

            // TODO - dat - LEAK LEAK LEAK
            // TODO - dat - LEAK LEAK LEAK

//            fread(p->list, 16, p->n, fp);
            fread(p->list, sizeof(pair64_t), p->m, fp);
 ...
 void download_from_remote(const char *bamURL, const char *localFN)
{
	...
	knetFile *fp_remote;
	int l;
	if (strstr(bamURL, "ftp://") != bamURL && strstr(bamURL, "http://") != bamURL) return;
	l = strlen(bamURL);

    char *idxURL = calloc(strlen(bamURL) + 5, 1);
    strcat(strcpy(idxURL, bamURL), ".bai");
    
    fprintf(stderr, "[bam_index_load] attempting to download the remote index file.: %s\n", idxURL);    
	fp_remote = knet_open(idxURL, "r");
    fprintf(stderr, "done.\n");
	...
	while ((l = knet_read(fp_remote, buf, buf_size)) != 0) {
        ...
        fprintf(stderr, "[download_from_remote] writing\n");
