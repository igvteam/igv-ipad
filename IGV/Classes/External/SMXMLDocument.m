...
static NSError *SMXMLDocumentError(NSXMLParser *parser, NSError *parseError) {
    ...
    [userInfo setObject:[NSString stringWithFormat:@"Malformed XML document. Error at line %@:%@.", lineNumber, columnNumber] forKey:NSLocalizedDescriptionKey];
...
- (NSString *)descriptionWithIndent:(NSString *)indent {
    ...
    for (NSString *attribute in attributes) [s appendFormat:@" %@=\"%@\"", attribute, [attributes objectForKey:attribute]];

    NSString *trimVal = [value stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if (trimVal.length > 25) trimVal = [NSString stringWithFormat:@"%@…", [trimVal substringToIndex:25]];
    ...
    if (children.count) {
        ...
        if (trimVal.length) [s appendFormat:@"%@%@\n", childIndent, trimVal];

        for (SMXMLElement *child in children) [s appendFormat:@"%@\n", [child descriptionWithIndent:childIndent]];
